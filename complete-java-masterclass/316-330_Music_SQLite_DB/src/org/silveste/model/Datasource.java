...
/*        finally {
    ...
    public List<SongArtist> queryArtistForSong(String songName, int sortOrder) {
        StringBuilder sb = new StringBuilder(QUERY_ARTIS_FOR_SONG_START);
        sb.append(songName);
        sb.append("\"");

        if (sortOrder != ORDER_BY_NONE){
            sb.append(QUERY_ARTIS_FOR_SONG_SORT);
            if(sortOrder == ORDER_BY_DESC){
                sb.append("DESC");
            } else {
                sb.append("ASC");
        ...
        try(Statement statement = conn.createStatement();
            ...
            while (results.next()){
                ...
                songArtist.setArtisName(results.getString(1));
    ...
    public List<SongArtist> querySongInfoView(String title){
        ...
        try {
            ...
            while (rs.next()){
                SongArtist songArtist = new SongArtist();
                songArtist.setArtisName(rs.getString(1));
...
//Code without PreparedStamements
//        try (Statement st = conn.createStatement();
